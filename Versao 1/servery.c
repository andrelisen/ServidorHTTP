...
{
    char *buffer;
    char *nfile = (char*)malloc((strlen(file_name)-1)*sizeof(char));
    //char *full_path = (char *)malloc((strlen(PATH) + strlen(file_name)) * sizeof(char));
    int fp;

    strcpy(nfile,&file_name[1]);
    printf("%s\n",nfile );
    ...
    if ((fp=open(nfile, O_RDONLY)) > 0) //FILE FOUND
    {
        puts("Image Found.");
        int bytes;
...
{
    char *buffer, *nfile;
    nfile = (char*)malloc((strlen(file_name)-1)*sizeof(char));
  //  char *full_path = (char *)malloc((strlen(PATH) + strlen(file_name)) * sizeof(char));
    FILE *fp;
